...
(define-public (register-project 
    ...
    (let ((project-id (var-get next-project-id)))
        ...
        (map-set projects project-id {
            ...
            project-start-date: block-height,
...
(define-public (issue-carbon-credits
    ...
    (let ((credit-id (var-get next-credit-id))
        ...
        (update-user-portfolio tx-sender credit-amount u0)
...
(define-public (create-sell-order
    ...
    (let ((credit (unwrap! (map-get? carbon-credits credit-id) ERR_CREDIT_NOT_FOUND))
        ...
        (update-order-book price-per-tonne "sell" quantity)
        (ok order-id)))
...
(define-public (create-buy-order
    ...
    (let ((order-id (var-get next-order-id))
        ...
        (var-set next-order-id (+ order-id u1))
        (update-order-book price-per-tonne "buy" quantity)
        (ok order-id)))
...
(define-public (execute-trade (buy-order-id uint) (sell-order-id uint) (trade-quantity uint))
    (let ((buy-order (unwrap! (map-get? trading-orders buy-order-id) ERR_ORDER_NOT_FOUND))
        ...
        (let ((trade-price (get price-per-tonne sell-order))
            ...
            (update-order-fill buy-order-id trade-quantity)
            (update-order-fill sell-order-id trade-quantity)
            
            ;; Update user portfolios
            (update-user-portfolio (get trader buy-order) trade-quantity u0)
            (update-user-portfolio (get trader sell-order) u0 u0)
...
(define-public (retire-carbon-credits
    ...
    (let ((credit (unwrap! (map-get? carbon-credits credit-id) ERR_CREDIT_NOT_FOUND)))
        ...
        (create-retirement-certificate credit-id tx-sender retirement-amount retirement-reason beneficiary)
        ...
        ;; Update user portfolio
        (update-user-portfolio tx-sender u0 retirement-amount)
